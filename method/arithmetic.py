...
def compress(uncompressed):
        ...
        for c in uncompressed:
                freqs.increment(ord(c))
        binary = ""
        # write frequencies
        for i in range(256):
                binary += "{0:08b}".format(freqs.get(i))
        bout = BinaryOutput()
        arithmetic = ArithmeticEncoder(8, bout)
        for c in uncompressed:
                arithmetic.update(freqs, ord(c))
        arithmetic.update(freqs, 256)  # EOF
        bout.write(1)  # Flush remaining code bits
...
def decompress(binary):
        # read frequencies
        freqs = [int(binary[i*8:i*8+8], 2) for i in range(256)]
        freqs.append(1)  # EOF symbol
        freqs = SimpleFrequencyTable(freqs)
        bin = BinaryInput(binary[256*8:])
        arithmetic = ArithmeticDecoder(8, bin)
        uncompressed = ""
        while True:
                symbol = arithmetic.read(freqs)
                if symbol == 256:  # EOF symbol
                        break
                uncompressed += chr(symbol)
        return [uncompressed, '']
